...
// lista de reduceri – luată din DB locală, filtrată pe cursă + visible_driver
...
// încărcăm reducerile din SQLite, pentru routeScheduleId
     LaunchedEffect(routeScheduleId) {
         if (routeScheduleId == null) {
             ...
             try {
                 val discounts = repo.getDiscountsForRouteSchedule(routeScheduleId)

                 val mapped = discounts.map { d ->
                     DiscountOption(
                         id = d.id,
                         label = d.label,
                         percent = d.valueOff   // la tine valueOff = procentul
                     )
                 }

                 // întotdeauna punem și „FĂRĂ REDUCERE” la început
                 options = listOf(
                     DiscountOption(
                         id = null,
                         label = "FĂRĂ REDUCERE",
                 ...
                 android.util.Log.e("ReduceriScreen", "Error loading discounts", e)
                 // fallback: doar „fără reducere”
                 options = listOf(
                     DiscountOption(
                         id = null,
                         label = "FĂRĂ REDUCERE",
