...
router.get('/trips/:tripId/my-seatmap', requireAuth, async (req, res) => {
  const tripId = parseInt(req.params.tripId, 10);
  const employeeId = Number(req.user?.id) || null;

  if (!Number.isInteger(tripId) || !employeeId) {
    return res.status(400).json({ ok: false, error: 'invalid_trip_or_user' });
  }

  try {
    // 1) găsim trip_vehicle + vehicle_id pentru șofer
    // Încearcă mai întâi prin trip_vehicle_employees (șofer atașat explicit)
    const tvSql = `
      SELECT tv.id AS trip_vehicle_id, tv.vehicle_id
      FROM trip_vehicle_employees tve
      JOIN trip_vehicles tv ON tv.id = tve.trip_vehicle_id
      WHERE tve.employee_id = ?
        AND tv.trip_id = ?
      ORDER BY tv.is_primary DESC, tv.id ASC
      LIMIT 1
    `;
    const { rows: tvRows } = await db.query(tvSql, [employeeId, tripId]);
    let tv = tvRows?.[0];
    ...
    // 2) seats layout
    const seatsSql = `
      SELECT id, vehicle_id, label, row, seat_col, seat_type, pair_id
      FROM seats
      WHERE vehicle_id = ?
      ORDER BY row, seat_col, id
    `;
    const { rows: seatRows } = await db.query(seatsSql, [tv.vehicle_id]);

    // 3) rezervările pentru acest trip + doar seats din vehicle
    const resSql = `
      SELECT
        r.id, r.trip_id, r.seat_id,
        r.person_id, p.name AS person_name, p.phone AS person_phone,
        r.status, r.board_station_id, r.exit_station_id,
        r.boarded, r.boarded_at,
        r.reservation_time, r.created_by AS agent_id, e.name AS agent_name,
        rp.net_price AS final_price,
        disc.discount_amount,
        CASE WHEN rp.net_price IS NULL THEN NULL ELSE rp.net_price + COALESCE(disc.discount_amount, 0) END AS base_price,
        pay.paid_amount,
        CASE WHEN rp.net_price IS NULL THEN NULL ELSE GREATEST(rp.net_price - COALESCE(pay.paid_amount, 0),0) END AS due_amount,
        CASE WHEN rp.net_price IS NULL THEN 0 WHEN rp.net_price <= COALESCE(pay.paid_amount,0) THEN 1 ELSE 0 END AS is_paid_raw
      FROM reservations r
      JOIN seats s ON s.id = r.seat_id AND s.vehicle_id = ?
      LEFT JOIN people p ON p.id = r.person_id
      LEFT JOIN employees e ON e.id = r.created_by
      LEFT JOIN (
        SELECT reservation_id, MAX(price_value) AS net_price
        FROM reservation_pricing
        GROUP BY reservation_id
      ...
      LEFT JOIN (
        ...
        FROM reservation_discounts
        GROUP BY reservation_id
      ) disc ON disc.reservation_id = r.id
      LEFT JOIN (
        SELECT reservation_id, SUM(CASE WHEN status='paid' THEN amount ELSE 0 END) AS paid_amount
        FROM payments
        GROUP BY reservation_id
      ...
      ORDER BY r.seat_id, r.id
    `;
    const { rows: resRows } = await db.query(resSql, [tv.vehicle_id, tripId]);

    const reservations = resRows.map(r => ({
      ...r,
      is_paid: !!r.is_paid_raw,
      is_paid_raw: undefined,
    }));

    return res.json({
      ok: true,
      trip_id: tripId,
      trip_vehicle_id: tv.trip_vehicle_id,
      vehicle_id: tv.vehicle_id,
      seats: seatRows,
      reservations,
    });
  } catch (err) {
    console.error('[mobile] my-seatmap error', err);
    return res.status(500).json({ ok: false, error: 'internal_error' });
  }
});
