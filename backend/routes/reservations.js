  ...
  try {
    ...
    for (const p of passengers) {
      ...
      if (existingReservation) {
        ...
        // ✅ Recalculează și salvează pricing/discount și la UPDATE (edit rezervare)
        // Motiv: prețul afișat vine din reservation_pricing + reservation_discounts,
        // iar până acum la update rămânea istoricul vechi.

        const resId = Number(p.reservation_id);

        // 1) Curățăm reducerile vechi (tip + promo) ca să evităm valori vechi/dubluri
        await db.query(`DELETE FROM reservation_discounts WHERE reservation_id = ?`, [resId]);

        // Curățăm și usages vechi ca să nu dublăm la edit
        // (nu ai UNIQUE, deci altfel se umflă usages la fiecare edit)
        await db.query(`DELETE FROM promo_code_usages WHERE reservation_id = ?`, [resId]);

        // 2) Discount de tip (elev/student/etc.)
        ...
        if (p.discount_type_id) {
          ...
          await db.query(
            ...
            VALUES (?, ?, ?, ?)
            `,
            [resId, disc.id, typeDiscountAmount, Number(disc.value_off)]
          );
        }

        // 3) Promo (dacă payload are promo_apply valid și încă există promoRemaining)
        // IMPORTANT: promoConn/promoMeta/promoRemaining sunt deja calculate mai sus în request
        let promoPiece = 0;
          ...
          if (baseAfterType > 0) {
            ...
            if (promoPiece > 0) {
              await promoConn.query(
                ...
                VALUES (?, NULL, ?, ?, ?)
                `,
                [resId, promoMeta.id, promoPiece, Number(promoMeta.value_off)]
              );
              ...
              await promoConn.query(
                ...
                VALUES (?, ?, ?, ?)
                `,
                [promoMeta.id, resId, phone || null, promoPiece]
              );
        ...
        // 4) Pricing net (după discount tip + promo)
        ...
        const upd = await db.query(
          ...
          `,
          [netPrice, listId, p.category_id, effChannel, Number(req.user?.id) || null, resId]
        );

        // dacă lipsește rândul (date vechi), facem insert
        if (upd.rowCount === 0) {
          await db.query(
            `
            ...
            VALUES (?, ?, ?, ?, ?, ?)
            `,
            [resId, netPrice, listId, p.category_id, effChannel, Number(req.user?.id) || null]
          );
        }
