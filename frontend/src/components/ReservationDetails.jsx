...
export default function ReservationDetails() {
  ...
  useEffect(() => {
    ...
    const url = `${apiUrl}/api/reservations/${id}/details`;

    fetch(url, { credentials: 'include' })
      .then(async (r) => {
        const ct = (r.headers.get('content-type') || '').toLowerCase();

        // Dacă nu e JSON, aproape sigur ai primit HTML (index/soon/login/redirect)
        if (!ct.includes('application/json')) {
          const text = await r.text().catch(() => '');
          throw new Error(
            `API nu a răspuns cu JSON (status ${r.status}). Cel mai probabil ai primit HTML (login/soon/index).`
          );
        }
     ...
     .then(j => { if (alive) { setData(j); setErr(''); } })
      .catch(e => { if (alive) setErr(String(e?.message || e || 'Eroare la încărcare detalii')); })
