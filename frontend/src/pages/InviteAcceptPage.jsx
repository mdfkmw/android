...
const API_BASE =
  (import.meta.env.VITE_API_URL || 'https://api.pris-com.ro').replace(/\/$/, '');
const apiUrl = (path) => {
  const p = String(path || '');
...
export default function InviteAcceptPage() {
  ...
  const readJsonOrThrow = async (res, fallbackMessage) => {
    ...
    // Dacă primești HTML (<!doctype...) în loc de JSON, aici îl prindem clar.
    if (!ct.includes('application/json')) {
      const txt = await res.text().catch(() => '');
      throw new Error(
        `${fallbackMessage || 'Răspuns invalid de la server.'} (non-JSON: ${ct || 'no content-type'})`
      );
    }
  ...
  useEffect(() => {
    ...
    fetch(apiUrl(`/api/invitations/${token}`))
      .then(async res => {
        if (!res.ok) {
          const data = await readJsonOrThrow(res, 'Invitația nu a fost găsită.');
          throw new Error(data?.error || 'Invitația nu a fost găsită.');
        }
        return readJsonOrThrow(res, 'Invitația nu a putut fi citită.');
      })
      .then(data => {
        if (cancelled) return;
        setInviteInfo(data);
  ...
  const handleSubmit = async event => {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await readJsonOrThrow(res, 'Nu am putut valida invitația.');
